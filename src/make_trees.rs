...
pub fn make_tiny(level: &mut Level, base_pos: Vec3, species: TreeSpecies) {
    ...
    level[pos + Vec3(1, 0, 0)] |= leaf_block;
    level[pos + Vec3(-1, 0, 0)] |= leaf_block;
    level[pos + Vec3(0, 1, 0)] |= leaf_block;
    level[pos + Vec3(0, -1, 0)] |= leaf_block;
    level[pos + Vec3(0, 0, 1)] |= leaf_block;
    level[pos + Vec3(0, 0, -1)] |= leaf_block;

    for leaf_pos in Cuboid::new(pos - Vec3(2, 2, 2), pos + Vec3(2, 2, 2)).iter() {
        let distance_squared = ((leaf_pos - pos).0 * (leaf_pos - pos).0
...
pub fn make_straight(level: &mut Level, pos: Vec3, species: TreeSpecies) {
    let log_block = Log(species, LogType::FullBark);
    let leaf_block = Leaves(species);
    ...
    for y in 1..=log_height {
        level[pos + Vec3(0, y, 0)] = log_block;
    }

    for Vec2(x, z) in &[Vec2(1, 0), Vec2(-1, 0), Vec2(0, 1), Vec2(0, -1)] {
        for y in 3..=log_height + 1 {
            level[pos + Vec3(*x, y, *z)] = leaf_block;
        }
    }

    level[pos + Vec3(0, log_height + 1, 0)] |= leaf_block;
    level[pos + Vec3(0, log_height + 2, 0)] |= leaf_block;
    if (log_height == 5) & rand(0.75) | (log_height > 5) {
        level[pos + Vec3(0, log_height + 3, 0)] |= leaf_block;
    }
