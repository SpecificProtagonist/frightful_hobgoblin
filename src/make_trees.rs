...
pub fn make_tiny(level: &mut Level, base_pos: IVec3, species: TreeSpecies) {
    ...
    let base_pos = base_pos + ivec3(0, 1, 0);
    level[base_pos] = log_block;

    let mut pos = base_pos + ivec3(0, 1, 0) + rand_2(0.2).d3();
    level[pos] = log_block;

    pos.x += 1;
    if rand(0.8) {
        if pos.d2() == base_pos.d2() {
            pos += rand_2(0.3).d3();
        }
        level[pos] = log_block;
    ...
    for leaf_pos in Cuboid::new(pos - IVec3::splat(2), pos + IVec3::splat(2)).iter() {
        let distance_squared = ((leaf_pos - pos).x * (leaf_pos - pos).x
            + (leaf_pos - pos).y * (leaf_pos - pos).y
            + (leaf_pos - pos).z * (leaf_pos - pos).z) as f32;
...
pub fn make_straight(level: &mut Level, pos: IVec3, species: TreeSpecies) {
    ...
    for y in 1..=log_height {
        level[pos + ivec3(0, y, 0)] = log_block;
    }
    ...
    level[pos + ivec3(0, log_height + 1, 0)] |= leaf_block;
    level[pos + ivec3(0, log_height + 2, 0)] |= leaf_block;
    if (log_height == 5) & rand(0.75) | (log_height > 5) {
        level[pos + ivec3(0, log_height + 3, 0)] |= leaf_block;
    }
