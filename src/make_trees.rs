...
pub fn make_tiny(world: &mut World, base_pos: Vec3, species: TreeSpecies) {
    ...
    world[pos + Vec3(1, 0, 0)] |= leaf_block;
    world[pos + Vec3(-1, 0, 0)] |= leaf_block;
    world[pos + Vec3(0, 1, 0)] |= leaf_block;
    world[pos + Vec3(0, -1, 0)] |= leaf_block;
    world[pos + Vec3(0, 0, 1)] |= leaf_block;
    world[pos + Vec3(0, 0, -1)] |= leaf_block;
...
pub fn make_straight(world: &mut World, pos: Vec3, species: TreeSpecies) {
    ...
    world[pos + Vec3(0, log_height + 1, 0)] |= leaf_block;
    world[pos + Vec3(0, log_height + 2, 0)] |= leaf_block;
    if (log_height == 5) & rand(0.75) | (log_height > 5) {
        world[pos + Vec3(0, log_height + 3, 0)] |= leaf_block;
    }
